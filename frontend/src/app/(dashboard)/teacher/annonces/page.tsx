...
export default function TeacherAnnoncesPage() {
  const [announcements, setAnnouncements] = useState<Announcement[]>([])
  const [loading, setLoading] = useState(true)
  const [showForm, setShowForm] = useState(false)
  const [selected, setSelected] = useState<Announcement | null>(null)
  const [form, setForm] = useState({ title: '', content: '', audience: 'students', is_pinned: false })
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false)
      setForm({ title: '', content: '', audience: 'students', is_pinned: false })
      fetchAnnouncements()
    } catch (err: any) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="flex items-center gap-2 pt-5">
              ...
              <label htmlFor="pin" className="text-sm text-gray-700 cursor-pointer flex items-center gap-1.5">
                <Pin className="w-3.5 h-3.5 text-amber-500" />Épingler l'annonce
              </label>
            </div>
          </div>
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-400">
                    Publié le {formatDate(selected.created_at)} · {AUDIENCE_LABELS[selected.audience] || selected.audience}
                  </p>
                </div>
                <div className="flex gap-2">
